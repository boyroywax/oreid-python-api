...
async def oauth2callback():
    ...
    if 'code' not in request.args:
        ...
        result = verified_discover.get("verified", False)
        ...
        if result is True:
            ...
            result = verified_token.get("verified", False)
            ...
            if result is True:
                ...
                result = verified_login.get("verified", False)

        # # userinfo_endpoint = google_provider_config["userinfo_endpoint"]
        # # api.setBaseUrl(userinfo_endpoint)
        # # api.setHeaders({'Authorization': f'Bearer {token_response["id_token"]}'})
        # # user_returned = await api.makeCall('GET', None, None, None)
        # # print(user_returned)
                ...
                if result is True:
                    ...
                    user_data = oreid_api.get_user(verified_login['response'].get("account", None))
                    print(f'user_data: {user_data}')
                    return redirect(url_for('user'))
                else:
                    return await render_template_string("Could not login")
    return await render_template_string("Please try again")
